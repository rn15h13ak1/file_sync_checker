...
7. [複数の比較を定義する](#5-複数の比較を定義する)
8. [定期実行する](#6-定期実行する)

**リファレンス**
9. [設定ファイル](#設定ファイル)
10. [CLI オプション一覧](#cli-オプション一覧)
11. [出力レポート構成](#出力レポート構成)
12. [ファイル名の突き合わせ](#ファイル名の突き合わせ)
...
| 比較対象を変更 | 設定に定義された比較を切り替える（2 つ以上あるときだけ表示）。実行だけなら「チェックを実行」から選べます |
...
**メニューからは番号で選べます。**「チェックを実行」を選ぶと、そのまま実行対象の
一覧が出ます。1 つ選ぶことも、まとめて実行することもできます。
...
**すべて実行**を選ぶと順に照合し、最後にまとめて結果を表示します。
...
============================================================
  契約書              0  全拠点一致・エラーなし
  設計                1  差分を検出 (スキャン自体は完走)
------------------------------------------------------------
  終了コード: 1 (差分を検出)
```

- 1 件が失敗しても残りは実行します（後続を確認できないと困るため）
- 中断したときだけ打ち切ります
- 表示する終了コードは最も重いもの（`中断 > 予期しないエラー > 読み取りエラー > 差分 > 一致`）
- 比較が 1 組しか無い設定では選択を挟まず、そのまま実行します
- 1 つだけ選んだ場合、以降の「設定を確認」「前回のレポートを開く」もその対象になります

- **レポートは比較ごとに分かれます**。`sync-check-契約書-YYYYMMDD-HHMMSS.html` と
  固定名の `sync-check-契約書.html` が出力され、互いに上書きしません
- **古いレポートの削除も比較ごとです**。`output.keep_reports` は比較ごとに N 回分を残します
- **比較の名前はレポートのファイル名に入る**ため、`\ / : * ? " < > |` は使えません
...
# 比較を複数定義している場合は、比較ごとに実行する
python3 /full/path/to/main.py -c /full/path/to/config.yaml --comparison 契約書
```
...
| `--comparison NAME` | - | 実行する比較を選ぶ（設定: `comparisons`） |
| `--list-comparisons` | - | 定義されている比較の名前を一覧表示して終了する |
...
- **`main.py` の 1 回の実行で照合できる比較は 1 組だけです。** 複数まとめて実行したい
  場合はメニューの「すべて実行」を使うか、`--comparison` を変えて繰り返し呼んでください。
